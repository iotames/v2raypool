...
func getConfByEnv() error {
	...
	ecf.StringVar(&cf.SubscribeUrl, "VP_SUBSCRIBE_URL", "", "代理节点订阅地址（与订阅数据文件二选一）", "如果只有类似Clash的.yml文件，没订阅地址。可直接填文件地址。", "与 VP_SUBSCRIBE_DATA_FILE 二选一，至少配置一项即可。")
	ecf.StringVar(&cf.SubscribeDataFile, "VP_SUBSCRIBE_DATA_FILE", conf.DEFAULT_SUBSCRIBE_DATA_FILE, "订阅数据文件（与订阅地址二选一）", "若订阅地址无法直接访问，可将订阅数据保存到此文件。", "支持Clash的.yml订阅文件。", "与 VP_SUBSCRIBE_URL 二选一，至少配置一项即可。")
