...
隧道代理池的核心是一个纯 Go 编写的 HTTP 代理服务器（`TunnelPool`），**它本身不是 v2ray 进程**，但复用了代理池中每个节点已启动的 v2ray 进程的本地 HTTP 端口。
...
┌────────────┐     CONNECT/HTTP      ┌────────────────────┐
│   客户端    │ ───────────────────→  │   隧道代理池        │
│   (爬虫)    │   http://127.0.0.1    │   TunnelPool       │
│             │        :1080          │   端口 1080        │
└────────────┘                        └─────────┬──────────┘
                                                 │
                               随机选择一个可用节点 │ pickNodeAddr()
                                                 ▼
                               ┌─────────────────────────────────┐
                               │ 节点 1 本地 HTTP :30001  ← v2ray│
                               │ 节点 2 本地 HTTP :30002  ← v2ray│
                               │ 节点 3 本地 HTTP :30003  ← v2ray│
                               │ ……                              │
                               └────────────┬────────────────────┘
                                            │ v2ray 内核经远程节点转发
                                            ▼
...
```
客户端 → TunnelPool(:1080)                                # 对外单一 HTTP 代理端口
         → pickNodeAddr() 随机选一个可用节点                # 每次请求独立选取
         → 连接该节点在本地的 HTTP 代理地址(如 127.0.0.1:300xx)  # v2ray 进程的 inbound 端口
         → v2ray 内核经远程节点转发                        # outbound 走远程代理出口
         → 目标服务器                                      # 出口 IP 随选取的节点变化
```

隧道池自身不运行 v2ray 进程，它通过复用 `ProxyPool` 中每个节点的本地 HTTP 端口来工作。节点由 `ProxyPool` 统一管理（订阅解析、v2ray 进程启停、测速），隧道池只负责**调度**——每来一个请求就从测速合格的节点池中随机选一个，把流量通过该节点的本地端口发出去，从而实现单端口 + 随机出口 IP 的效果。

测速由隧道池后台定期（默认 20 分钟）触发，结果写入全局 `speedMap`，与原有代理池共享，避免重复测速。HTTPS 走 `CONNECT` 隧道透传，HTTP 走常规代理转发。
...
# 代理节点订阅地址（二选一：订阅地址或订阅数据文件至少配置其一）
# 支持 http:// 和 https:// 协议。也支持本地 .yml/.yaml 文件路径。
VP_SUBSCRIBE_URL = ""

# 订阅数据文件
# 若订阅地址无法直接访问，可将订阅数据保存到此文件中。
# 若有设置订阅数据文件，且文件内容不为空，则优先从该文件读取订阅节点信息。
# 支持Clash的.yml订阅文件。
# 与 VP_SUBSCRIBE_URL 二选一，至少配置一项即可。
